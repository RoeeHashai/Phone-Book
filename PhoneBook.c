...
int contactExist(Contact *scannerPtr, Contact *newContact);
...
Contact *
searchContactByName(Contact *phonebook, char firstName[MAX_ARRAY_LENGTH],
                    char lastName[MAX_ARRAY_LENGTH]);
...
int main() {
    ...
    // prints the welcome prompt to user
    printWelcomeMessage();
    do {
...
***********************************************************************/
int contactExist(Contact *scannerPtr, Contact *newContact) {
    while (scannerPtr != NULL) {
        if (!strcmp(scannerPtr->firstName, newContact->firstName) &&
            !strcmp(scannerPtr->lastName, newContact->lastName)) {
            return NAME_ERROR;
        } else if (!strcmp(scannerPtr->phoneNum, newContact->phoneNum)) {
            return PHONE_NUMBER_ERROR;
        } else
            scannerPtr = scannerPtr->next;
...
void addNewContact(Contact *phonebook[ABC_LENGTH]) {
    ...
    contactExistence = contactExist(phonebook[firstLetterIndex],
                                    ptrNewContact);
...
void deleteContact(Contact *phonebook[ABC_LENGTH]) {
    ...
    if (!strcmp(ptrCurrentContact->lastName, lastName) &&
